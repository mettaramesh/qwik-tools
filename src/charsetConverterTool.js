...
export function loadCharsetConverterTool(container) {
  ...
  function updateOutputModeDisplay() {
    if (outputModeDisplay) {
      ...
      outputModeDisplay.innerHTML = `Export as <strong>${outputText[outputAs?.value] || 'Text'}</strong>`;
    }
  }
  ...
  function decodeBytes(bytes, label){
    if (label==='ibm-037') return decodeCP037(bytes, normalizeNEL?.checked || false);
    ...
    btnConvert.addEventListener('click', () => {
      try {
        ...
        if (mode==='text'){
          ...
          decoded=decodeBytes(bytes, byteDecode?.value || 'ibm-037');
        } else {
        ...
        const outKind=outputAs?.value || 'text';
        const enc=encodeWith?.value || 'utf-8';
        const withBOM=addBOM?.checked || false;
        ...
        if (outputArea) outputArea.value=outStr;
        lastBytes=outBytes;
  ...
  if (fileInput) {
    fileInput.addEventListener('change', async () => {
      ...
      fileInput._cachedBytes=null;
      const f=fileInput.files && fileInput.files[0]; if (!f) return;
      const arr=new Uint8Array(await f.arrayBuffer());
      fileInput._cachedBytes=arr;
      if (fileInfo) fileInfo.textContent = f.name + " - " + arr.length + " bytes";
    });
  }
  ...
  if (btnCopy) {
    btnCopy.addEventListener('click', async () => {
      try {
        if (navigator.clipboard && window.isSecureContext && outputArea) {
  ...
  if (btnDownload) {
    btnDownload.addEventListener('click', () => {
      const kind=outputAs?.value || 'text'; let blob,name;
      if (kind==='download'){
  ...
  // Capability note
  if (!hasTextDecoder && compatBox) {
    const warn=document.createElement('div');
    warn.className='compat-warning';
  ...
  // UX helpers
  if (btnSwap && inputArea && outputArea) {
  ...
  if (btnClear) {
    btnClear.addEventListener('click', () => {
      ...
      if (fileInfo) fileInfo.textContent=''; 
      if (inMetrics) inMetrics.innerHTML=''; 
      if (outMetrics) outMetrics.innerHTML=''; 
    });
  }
